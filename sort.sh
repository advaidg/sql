...
# Step 1: Remove `transactionNumber` (correctly spelled and case-sensitive) from anywhere in the first JSON file
jq 'walk(if type == "object" then del(.transactionNumber) else . end)' input.json | jq --sort-keys . > modified_input.json

# Step 2: Remove `transactionNumber` from anywhere in the second JSON file
